...
struct equal_to<::zsearch::ASAtom> {
  ...
  result_type operator()(const first_argument_type& lhs,
    ...
    if (!(lhs.asn() == rhs.asn() && lhs.description() == rhs.description() &&
          lhs.rir() == rhs.rir())) {
      return false;
    }
...
class Store {
 ...
 public:
  ...
  class StoreIterator {
   ...
   private:
    using zdb_iterator =
        typename DB<key_type, ProtobufRecord<zsearch::Record>>::iterator;
...
StoreResult Store<key_type>::put(zsearch::Record value) {
  ...
  for (auto it = m_db->upper_bound_prefix_seek(zero_subkey); it.valid(); ++it) {
  ...
  if (!seen_subkey) {
    ...
    m_public_location->populate_atom(public_location.mutable_public_location(),
                                     value.ip());
    ...
    private_location.set_subprotocol(zsearch::SUBPROTO_SYS_RESTRICTED_LOCATION);
    ...
    processing_threads.emplace_back(
        [&](size_t thread_id) {
          ...
          for (auto it = to_process.range(); it.valid(); ++it) {
            ...
            std::for_each(
                serialized_records.begin(), serialized_records.end(),
                [&](const std::string& s) {
                  // Deserialize
                  zsearch::Record record;
                  bool ok = record.ParseFromString(s);
                  if (!ok) {
                    log_error("store", "unable to deserialize record");
                    return;
                  }

                  // Prune here
                  PruneCheck pc = Store<key_type>::should_prune(
                      record, dump_config.min_scan_ids);
                  if (pc.should_prune) {
                    auto delta = this->del(key_type::from_record(record));
                    dump_config.prune_handlers[thread_id]->handle_delta(delta);
                    ++prune_stats.records_pruned;
                    prune_stats.pruned_per_key[pc.anon_key] += 1;
                    return;
                  }
            ...
            if (records.front().domain().size() == 0 &&
              ...
              auto as_record_it = std::find_if(
                  records.begin(), records.end(), [](const zsearch::Record& r) {
                    if (r.data_oneof_case() !=
                        zsearch::Record::DataOneofCase::kAsAtom) {
                      return false;
                    }
                    return r.port() == 0 &&
                           r.protocol() == zsearch::PROTO_SYSTEM &&
                           r.subprotocol() == zsearch::SUBPROTO_SYS_AS;
                  });
              ...
              if (as_record_it != records.end()) {
                ...
                auto as_res = this->m_as_handle[ntohl(as_record.ip())];
                if (as_res.found &&
                    !(equal_atom(as_record.as_atom(), as_res.as_atom))) {
                  as_record.mutable_as_atom()->Swap(&as_res.as_atom);
                }
                this->put(as_record);
              } else {
                // If we didn't find AS data, make a record
                auto as_res = this->m_as_handle[ntohl(records.front().ip())];
                if (as_res.found) {
                  zsearch::Record as_record;
            ...
            std::stringstream ss;
            bool non_empty =
                Store<key_type>::dump_host(ss, json_writer, std::move(records));
            if (non_empty) {
...
uint64_t Store<key_type>::regenerate_deltas(key_type start,
  ...
  for (auto it = host_upper_bound(start); it.valid(); ++it) {
    ...
    std::for_each(it->second.begin(), it->second.end(), [&](zsearch::Record r) {
      ...
      if (data_oneof == zsearch::Record::DataOneofCase::kPublicLocation) {
        ...
        if (r.public_location().latitude() == 0 &&
          ...
          m_public_location->populate_atom(
              public_location.mutable_public_location(), current_ip);
        ...
        if (r.private_location().latitude() == 0 &&
          ...
          m_restricted_location->populate_atom(
              private_location.mutable_private_location(), current_ip);
...
bool Store<key_type>::dump_host(std::ostream& out_stream,
  ...
  std::for_each(records.begin(), records.end(), [&](zsearch::Record& record) {
    ...
    if (record.protocol() == zsearch::PROTO_SYSTEM) {
      ...
      } else if (record.subprotocol() == zsearch::SUBPROTO_SYS_ALEXA_RANK) {
