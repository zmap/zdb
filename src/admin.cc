...
class AdminServiceImpl final : public zsearch::AdminService::Service {
 private:
  ...
  grpc::Status dump(const Command* request,
    ...
    log_info("admin", "dump max queue size %llu", dump_config.max_queue_size);
    ...
    for (MinScanId min_id : request->min_scan_ids()) {
      ...
      log_info(
          "admin", "will prune port %u, protocol %u, subprotocol %u to min %u",
          key.port(), key.protocol(), key.subprotocol(), min_id.min_scan_id());
    }
 ...
 public:
  ...
  virtual grpc::Status UpdateLocationData(grpc::ServerContext* context,
    ...
    log_info("admin", "location update will start at %s", start_str.c_str());
  ...
  virtual grpc::Status FixCertificateSource(ServerContext* context,
    ...
    for (auto it = cert_store.begin(); it.valid(); ++it, ++check_count) {
      ...
      if (check_count && check_count % 100000 == 0) {
        log_info("admin", "checked %llu certificates, fixed %llu", check_count,
                 fix_count);
      }
      if (rec.certificate().source() == zsearch::CERTIFICATE_SOURCE_RESERVED) {
  ...
  virtual grpc::Status DumpIPv4ToJSON(ServerContext* context,
    ...
    for (size_t i = 0; i < 12; ++i) {
      ipv4_delta_handlers.push_back(m_delta_ctx->new_ipv4_delta_handler());
    }
    auto status =
        dump(request, response, ipv4_store, std::move(ipv4_delta_handlers));
  ...
  virtual grpc::Status DumpDomainToJSON(ServerContext* context,
    ...
    for (size_t i = 0; i < 12; ++i) {
      domain_delta_handlers.push_back(m_delta_ctx->new_domain_delta_handler());
    }
    auto status =
        dump(request, response, domain_store, std::move(domain_delta_handlers));
  ...
  void DumpCertificateShardToJSON(size_t shard_id,
    ...
    for (; it.valid() && continue_iterating(it->first); ++it) {
      ...
      if (it->second.certificate().not_valid_before() &&
        ...
        bool expired = !certificate_valid_at(it->second.certificate(), now);
      ...
      std::string serialized = dump_certificate_to_json_string(it->second);
  ...
  void DumpCertificateThread(Channel<size_t>* shards_to_process,
    ...
    for (auto it = shards_to_process->range(); it.valid(); ++it) {
      ...
      log_debug("admin", "dump certificate thread starting to process %llu",
                target_shard);
      DumpCertificateShardToJSON(target_shard, start_shard, start, stop_shard,
                                 has_stop, stop, out, out_mtx, max_records,
                                 count);
    }
  }

  virtual grpc::Status DumpCertificatesToJSON(ServerContext* context,
                                              const Command* request,
                                              CommandReply* response) override {
    ...
    log_info("admin", "will dump certificates using %u threads", num_threads);
    ...
    HashKey start = HashKey::zero_pad_prefix(start_prefix, HashKey::SHA256_LEN);
    HashKey stop = HashKey::zero_pad_prefix(end_prefix, HashKey::SHA256_LEN);
    ...
    for (uint32_t i = 0; i < num_threads; i++) {
      threads.emplace_back(std::thread(
          &AdminServiceImpl::DumpCertificateThread, this, &shards_to_process,
          start_shard, start, stop_shard, has_stop, stop, std::ref(f),
  ...
  virtual grpc::Status RegenerateIPv4Deltas(ServerContext* context,
    ...
    auto start_str = make_ip_str(request->start_ip());
    auto stop_str = make_ip_str(request->stop_ip());
    log_info("admin", "delta regeneration will start at %s", start_str.c_str());
    log_info("admin", "delta regeneration will stop at %s", stop_str.c_str());
    IPv4Key start{request->start_ip(), 0, 0, 0};
    IPv4Key stop{request->stop_ip(), 0, 0, 0};
  ...
  virtual grpc::Status RegenerateDomainDeltas(ServerContext* context,
                                              const Command* request,
                                              CommandReply* response) override {
    ...
    uint64_t count =
        domain_store.regenerate_deltas(start, stop, *domain_delta_handler);
    log_info("admin", "generated %llu deltas", count);
    response->set_status(CommandReply::SUCCESS);
    return grpc::Status::OK;
  }
  ...
  virtual grpc::Status ReprocessCertificates(ServerContext* context,
                                             const Command* request,
                                             CommandReply* response) override {
    log_info("admin", "starting to queue all certificates for reprocessing");
    size_t max_records = request->max_records();
    if (max_records) {
...
std::unique_ptr<zdb::AdminServer> zdb::make_admin_server(
  ...
  unique_ptr<AdminServiceImpl> service(new AdminServiceImpl(
      std::move(ipv4), std::move(domain), std::move(certificate), delta_ctx,
      store_ctx->as_tree()));
