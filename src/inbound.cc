 ...
 public:
  ...
  InboundResult handle(const string& serialized) {
    ...
    if (!res.success) {
      ...
      log_error("inbound", "could not write record at key %s", key_str.c_str());
      return InboundResult{false, ""};
    }
...
class ProcessedCertificateHandler : public InboundHandler {
 ...
 public:
  ProcessedCertificateHandler(AnonymousStore<key_type>&& store)
      : m_store(std::move(store)) {}
  ...
  InboundResult handle(const string& serialized) {
    ...
    if (!certificate.ParseFromString(serialized)) {
      ...
      log_error("processed_cert handler",
                "could not deserialize Certificate record: %s", hex.c_str());
      return InboundResult{false, ""};
    }
    if (certificate.sha256fp().empty()) {
      auto hex = hex_encode(certificate.raw());
      log_error("processed_cert handler", "certificate had empty sha256fp: %s",
                hex.c_str());
      return InboundResult{true, ""};
    }
    ...
    if (recv_result.status == KafkaResult::Status::OK) {
      ...
      if (failcount >= 100) {
        log_info("inbound", "empty queue %s, %s, sleeping",
                 recv_topic->topic_name().c_str(), recv_result.error.c_str());
        sleep(10);
        failcount = 0;
      }
    } else {
...
vector<InboundOptions> zdb::configure_inbound(ConfigValues* config_values,
  ...
  if (config_values->domain.enabled &&
    ...
    for (size_t i = 0; i < domain.threads; ++i) {
      ...
      domain.handlers.back().reset(new StoreHandler<DomainKey>(std::move(s)));
    }
  ...
  if (config_values->certificate.enabled &&
    ...
    for (size_t i = 0; i < certificate.threads; ++i) {
      ...
      certificate.handlers.back().reset(new AnonHandler<HashKey>(std::move(s)));
    }
  ...
  if (config_values->external_certificate.enabled &&
    ...
    if (config_values->certificate.enabled) {
      external_certificate_offset += config_values->certificate.worker_threads;
    }
  ...
  if (config_values->processed_cert.enabled &&
      config_values->processed_cert.worker_threads > 0) {
    ...
    for (size_t i = 0; i < processed_cert.threads; ++i) {
      ...
      processed_cert.handlers.back().reset(
          new ProcessedCertificateHandler<HashKey>(std::move(s)));
    }
