...
bool GeoIP::open(const std::string& path) {
    ...
    if (!m_open) {
        log_error("location", "unable to open location data at %s",
                  path.c_str());
    }
    return m_open;
}
...
bool GeoIP::populate_atom(zsearch::LocationAtom* atom, uint32_t ip) const {
    ...
    int status = MMDB_get_value(&result.entry, &entry_data, "city", "names",
                                "en", NULL);
    ...
    status = MMDB_get_value(&result.entry, &entry_data, "city", "names", "en",
                            NULL);
    if (status == MMDB_SUCCESS && entry_data.has_data) {
        atom->set_city(
                std::string(entry_data.utf8_string, entry_data.data_size));
    ...
    status = MMDB_get_value(&result.entry, &entry_data, "location", "time_zone",
                            NULL);
    if (status == MMDB_SUCCESS && entry_data.has_data) {
        atom->set_timezone(
                std::string(entry_data.utf8_string, entry_data.data_size));
    ...
    status = MMDB_get_value(&result.entry, &entry_data, "location", "latitude",
                            NULL);
    if (status == MMDB_SUCCESS && entry_data.has_data) {
        atom->set_latitude(entry_data.double_value);
    }

    status = MMDB_get_value(&result.entry, &entry_data, "location", "longitude",
                            NULL);
    if (status == MMDB_SUCCESS && entry_data.has_data) {
