...
bool config_read_basic_database(const Json::Value& subconfig,
    ...
    if (!config_read_string(c, DB_PATH_KEY, &out->db_path)) {
        log_error("config", "could not read %s for %s", DB_PATH_KEY,
                  key.c_str());
        return false;
    }
    ...
    if (!config_read_optional_boolean(c, ENABLED_KEY, &out->enabled)) {
        log_error("config", "could not read %s for %s", ENABLED_KEY,
                  key.c_str());
        return false;
    }
    return true;
...
bool ConfigValues::from_json(const Json::Value& config, ConfigValues* out) {
    ...
    if (!config_read_inbound_source(queue_config, "processed_certs", &out->processed_cert)) {
        return false;
    }
