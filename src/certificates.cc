...
bool zdb::valid_parent(X509* parent, X509* child) {
...
static inline bool check_self_signed(X509* c) {
    return zdb::valid_parent(c, c);
}

X509* zdb::certificate_to_x509(const zsearch::Certificate& cert) {
...
zdb::CertificateSubjectIssuer zdb::x509_get_subject_issuer(X509* c) {
...
std::string zdb::x509_get_fingerprint_sha256(X509* c) {
...
std::string zdb::x509_get_subject_string(X509* c) {
...
std::string zdb::x509_get_issuer_string(X509* c) {
...
bool zdb::x509_check_time_valid_now(X509* c) {
