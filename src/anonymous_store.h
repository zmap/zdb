 ...
 public:
  ...
  class AnonIterator {
   ...
   private:
    ...
    AnonIterator(zdb_iterator&& it) : m_it(std::move(it)) { update_current(); }
...
AnonymousResult AnonymousStore<key_type>::put_external(
  ...
  try {
    ...
    log_error("anonstore", "unknown exception thrown by db in put_external");
    return AnonymousResult::failure();
  }
  ...
  if (retv.success && (ctrm.source() == zsearch::CERTIFICATE_SOURCE_CT ||
                       ctrm.source() == zsearch::CERTIFICATE_SOURCE_CT_CHAIN)) {
    cachehash_put(m_cache, ch_key, ch_len, this);
  }
